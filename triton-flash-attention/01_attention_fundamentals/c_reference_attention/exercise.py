...
def reference_attention(
    ...
    # Step 1: Extract head dimension (d_k) from Q's last axis
    head_dim = Q.shape[-1]

    # Step 2: Default scale = 1/sqrt(d_k), keeps dot-product variance ≈ 1
    if softmax_scale is None:
        softmax_scale = 1.0 / math.sqrt(head_dim)

    # Step 3: Compute scaled attention scores: S = Q K^T / sqrt(d_k)
    # Shape: (BATCH, HEADS, SEQ_LEN, SEQ_LEN) — the full N×N attention matrix
    scores = Q @ K.transpose(-2, -1) * softmax_scale

    # Step 4: Causal mask — position i can only attend to positions j <= i
    # Upper-triangle entries are set to -inf so softmax maps them to 0
    if causal:
        SEQ_LEN = Q.shape[-2]
        mask = torch.tril(torch.ones(SEQ_LEN, SEQ_LEN, device=Q.device))
        scores = scores.masked_fill(mask == 0, float("-inf"))

    # Step 5: Softmax along the key dimension (last axis) to get attention weights
    weights = F.softmax(scores, dim=-1)

    # Step 6: Weighted sum of value vectors: O = softmax(S) @ V
    out = weights @ V

    # Step 7: Return output, same shape as Q: (BATCH, HEADS, SEQ_LEN, HEAD_DIM)
    return out
