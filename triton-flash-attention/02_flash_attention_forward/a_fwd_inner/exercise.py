...
def _attn_fwd_inner(
    ...
    """
    # --- Part A: Set range based on STAGE ---
    if STAGE == 1:
    ...
    # --- Part B: Loop over KV blocks ---
    for start_kv in range(lo, hi, BLOCK_SIZE_KV):
        start_kv = tl.multiple_of(start_kv, BLOCK_SIZE_KV)

        # Step 1-2: Load K and compute QK scores
        K_block = tl.load(K_block_ptr)
        QK_block = tl.dot(Q_block, K_block)

        # Step 3: Online softmax with optional causal mask
        if STAGE == 2:
            mask = offs_q[:, None] >= (start_kv + offs_kv[None, :])
            QK_block = QK_block * softmax_scale + tl.where(mask, 0, -1.0e6)
            m_ij = tl.maximum(m_i, tl.max(QK_block, 1))
        ...
        # Steps 4-7: Attention weights and running sum update
        P_block = tl.math.exp(QK_block)
        l_ij = tl.sum(P_block, 1)
        alpha = tl.math.exp(m_i - m_ij)
        l_i = l_i * alpha + l_ij

        # Steps 8-12: Accumulate output
